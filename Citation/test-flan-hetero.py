...
from utils import load_data, accuracy, get_dataset, row_l1_normalize, GCN_norm, get_dataset_split, pairwise_distance, \
    convert_tensor_to_sparse_tensor, random_coauthor_amazon_splits
from gcn.models2 import LAGCN, CLAGCN, GCN
...
adj_normalized = GCN_norm(adj, features, slef_loop=False)
...
for i in trange(args.runs, desc='Run Train'):
    ...
    # idx_train, idx_val, idx_test = random_coauthor_amazon_splits(data.y, max(data.y)+1, None)
    idx_train = idx_train.to(device)
    idx_val = idx_val.to(device)
    idx_test = idx_test.to(device)

    X_MLP = torch.load("./X_MLP/" + args.dataset + "_X_MLP"+str(i)+".pt")
