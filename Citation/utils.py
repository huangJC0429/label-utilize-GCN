...
def random_coauthor_amazon_splits(y, num_classes, lcc_mask):
    ...
    if lcc_mask is not None:
        ...
        for i in range(num_classes):
            index = (y == i).nonzero().contiguous().view(-1)
            index = index[torch.randperm(index.size(0))]
            indices.append(index)
...
def GCN_norm(edge_index, x, slef_loop=True):
    row, col = edge_index
    num_nodes = x.shape[0]

    adj = SparseTensor(row=row, col=col, sparse_sizes=(num_nodes, num_nodes))
    # print(adj.is_symmetric())
    # exit()
    adj_norm = gcn_norm(adj.to_symmetric(),add_self_loops=slef_loop)
    return adj_norm
...
def convert_tensor_to_sparse_tensor(X):
    nonzero_indices = torch.nonzero(X)
    # 提取非零元素的值
    nonzero_values = X[nonzero_indices[:, 0], nonzero_indices[:, 1]]
    sp_adj = SparseTensor(row=nonzero_indices[:, 0], col=nonzero_indices[:, 1], value=nonzero_values, sparse_sizes=(X.shape[0], X.shape[0]))
    return sp_adj