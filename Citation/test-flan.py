...
from utils import load_data, accuracy, get_dataset, row_l1_normalize, GCN_norm, get_dataset_split, pairwise_distance, \
    convert_tensor_to_sparse_tensor
...
for i in trange(args.runs, desc='Run Train'):
    ...
    # for unaligned nodes
    test_UNF_mask = (~F_mask) & idx_test
    acc_unf_nodes = accuracy(output[test_UNF_mask], labels[test_UNF_mask])
    all_unf.append(acc_unf_nodes.item())
